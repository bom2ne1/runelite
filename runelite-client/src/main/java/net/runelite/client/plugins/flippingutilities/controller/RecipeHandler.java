...
import net.runelite.client.plugins.flippingutilities.model.FlippingItem;
import net.runelite.client.plugins.flippingutilities.model.OfferEvent;
import net.runelite.client.plugins.flippingutilities.model.PartialOffer;
import net.runelite.client.plugins.flippingutilities.model.RecipeFlip;
import net.runelite.client.plugins.flippingutilities.model.RecipeFlipGroup;
import net.runelite.client.plugins.flippingutilities.utilities.MathUtils;
import net.runelite.client.plugins.flippingutilities.utilities.PotionDose;
import net.runelite.client.plugins.flippingutilities.utilities.PotionGroup;
import net.runelite.client.plugins.flippingutilities.utilities.Recipe;
import net.runelite.client.plugins.flippingutilities.utilities.RecipeItem;
import net.runelite.client.plugins.flippingutilities.utilities.SORT;
...
public class RecipeHandler {
    ...
    // TODO this is pretty inefficient, i don't think it really matters, but might
    // be worth looking into.
    // TODO We could either cache it in the plugin or make the lookups faster
    // somehow.
    /**
     * Gets a map of offer id to partial offer. Since an offer can be referenced by
     * multiple partial offers (each
     * consuming part of the offer) from recipe flips in different recipe flip
     * groups, we need to sum up the amount
     * consumed by each of the partial offers referencing that one offer to get the
     * final partial offer corresponding
     * to that offer id.
     */
    ...
    /**
     * Gets the applicable recipe given an item id and whether you are
     * buying/selling the item.
     * For example, if you are buying a guthan warspear, the applicable recipe is
     * the one where the
     * warspear in the inputs and the guthan set is in the outputs. However, if you
     * were selling the
     * warspear, the applicable recipe would be the one where the guthan set was in
     * the inputs and the warspear
     * was in the outputs.
     */
    ...
    private List<Recipe> getApplicableDecantRecipes(int itemId, boolean isBuy) {
        ...
        List<PotionDose> otherDoses = potionGroup.getDoses().stream().filter(d -> d.getId() != itemId)
                .collect(Collectors.toList());
        for (PotionDose otherDose : otherDoses) {
            ...
            String recipeName = String.format("Decanting %s (%d)->(%d)", potionGroup.getName(), inputDose.getDose(),
                    outputDose.getDose());
    ...
    /**
     * Elemental runes need to be stripped bc people don't buy elemental runes for
     * recipes, they just use
     * staffs
     */
    ...
    private Optional<Map<Integer, PotionGroup>> getItemIdToPotionGroup(
            Optional<List<PotionGroup>> optionalPotionGroups) {
    ...
    /**
     * This method computes the initial target values for each of the items when the
     * panel
     * first shows up. This is so that the user doesn't have to manually input them
     * (tho they can still
     * adjust them if they want). The target values are selected such that the max
     * amount of
     * recipes can be made. Here is an example:
     *
     * Lets say there is a recipe where for every item C you need 3 of item A and 5
     * of item B. We
     * can shorten it by saying the recipe is 1C, 3A, and 5B. Now, lets say we have
     * a quantity of 5 for item C,
     * quantity of 9 for item A and a quantity of 25 for item B. What is the max
     * amount of this recipe you can make?
     * The max amount is only 3 because it is limited by that fact you only have 9 A
     * items, even though the amount
     * of C and B items you have can support more recipes.
     *
     * Once we know the max amount of recipes the offers can support, we can
     * multiply the max amount by the
     * amount the recipe needs of each item to know how much of each item we will
     * need to make the max amount of
     * recipes. Continuing with the example above, our max recipes is 3. To get the
     * amount of C items, we would do
     * 1 * 3. To get the amount of B items, we would do 5 * 3. To get the amount of
     * A items, we would do 3 * 3.
     *
     * In short:
     * recipe = 3A, 5B, 1C
    ...
    public Map<Integer, Integer> getTargetValuesForMaxRecipeCount(
        ...
        Map<Integer, Integer> itemIdToMaxRecipesThatCanBeMade = getItemIdToMaxRecipesThatCanBeMade(recipe,
                itemIdToPartialOffers, useRemainingOffer);
        int lowestRecipeCountThatCanBeMade = itemIdToMaxRecipesThatCanBeMade.values().stream()
                .min(Comparator.comparingInt(i -> i)).get();
        // if there are items with such few buys/sells that a recipe can't be made,
        // avoid returning target values of 0
        // as we still want to make it clear what the min amount needed for at least one
        // recipe creation is
    ...
    /**
     * Gets a mapping of item id to the max amount of recipes that can be
     * contributed to for each item in the given
     * partial offers
     */
    public Map<Integer, Integer> getItemIdToMaxRecipesThatCanBeMade(
        ...
        return itemIdToPartialOffers.entrySet().stream().map(e -> {
            ...
            long totalQuantity = e.getValue().stream().mapToLong(
                    po -> useRemainingOffer ? po.getOffer().getCurrentQuantityInTrade() - po.amountConsumed
                            : po.amountConsumed)
                    .sum();
    ...
    public List<RecipeFlipGroup> sortRecipeFlipGroups(List<RecipeFlipGroup> items, SORT selectedSort,
    ...
    public void deleteInvalidRecipeFlips(List<OfferEvent> offers, List<RecipeFlipGroup> recipeFlipGroups) {
        ...
        // get all the rfgs containing recipe flips that the offers could possibly be
        // referenced in
        recipeFlipGroups.stream().filter(rfg -> rfg.isInGroup(itemId))
                .forEach(rfg -> rfg.deleteFlipsWithDeletedOffers(offers));
    }
    ...
    private <T> CompletableFuture<Optional<T>> makeRequestAsync(String url, TypeToken<T> typeToken) {
        ...
        httpClient.newCall(request).enqueue(new Callback() {
            ...
            public void onResponse(Call call, Response response) {
                try (Response res = response) {
                    if (!res.isSuccessful()) {
                        log.error("Recipe fetch returned unsuccessful response: " + res);
                        future.complete(Optional.empty());
                        return;
                    }
                    if (res.body() == null) {
                        log.error("Recipe response body was null: " + res);
                        future.complete(Optional.empty());
                        return;
                    }
                    Type type = typeToken.getType();
                    T result = gson.fromJson(res.body().string(), type);
                    future.complete(Optional.of(result));
                } catch (Exception e) {
                    log.warn("Exception when processing recipe response: {}", ExceptionUtils.getStackTrace(e));
                    future.complete(Optional.empty());
                }
            }
        });
